...
from util.array_util import get_most_common_element
...
def calculate_average_general_ranking(history: ExecutionHistory):
    if SelectionMode.WEIGHT in history.get_available_seletion_modes():
        return _calculate_average_general_ranking_from_weights(history)
    elif SelectionMode.RANK in history.get_available_seletion_modes():
...
def _calculate_average_general_ranking_from_ranks(history: ExecutionHistory):
    average_rank = []
    accumulated_rank = []
    for i in range(0, history.get_n_features()):
