...
from config.general_config import OUTPUT_PATH, EXECUTION_TIME_OUTPUT_SUB_PATH
...
def _calculate_execution_time_statistics(execution_times_by_selector: dict[str, list[float]]):
    ...
    for selector, execution_times in execution_times_by_selector.items():
...
def _create_execution_average_table(execution_times_by_selector: dict[str, list[float]]):
    ...
    data = []
    for selector, statistics in execution_time_statistics_by_selector.items():
...
def _create_execution_time_chart(execution_times_by_selector: dict[str, list[float]]):
    ...
    labels = list(execution_times_by_selector.keys())
