...
from config.general_config import CREATE_HEATMAP_BASED_ON_INFORMATIVE_FEATURES, FEATURES_TO_DISPLAY_ON_GENERAL_HEATMAP, FEATURES_TO_DISPLAY_PLUS_INFORMATIVE_ON_HEADMAP, INFORMATIVE_FEATURES_STEP_ON_CHART, SHOULD_CALCULATE_METRICS_BY_LABEL, OUTPUT_PATH, INFORMATIVE_FEATURES_OUTPUT_SUB_PATH, MAX_INFORMATIVE_FEATURES_CHART_RANGE
...
from util.numpy_util import get_interception_len, normalize
...
def calculate_informative_features_scores(history: ExecutionHistory, dataset: Dataset) -> List[InformativeFeaturesScore]:
    ...
    for selection_size in range(1, limit_range):
        ...
        if SHOULD_CALCULATE_METRICS_BY_LABEL and informative_features_by_label_are_available_on_dataset and SelectionSpecificity.PER_LABEL in history.get_selection_specificities():
...
def create_heatmap(selector: BaseWeightSelectorWrapper, dataset: Dataset):
    if selector.get_selection_mode() == SelectionMode.WEIGHT:
        ...
        if CREATE_HEATMAP_BASED_ON_INFORMATIVE_FEATURES and knows_informative_features:
            ...
            if SelectionSpecificity.GENERAL in selector.get_selection_specificities():
            ...
            if SelectionSpecificity.PER_LABEL in selector.get_selection_specificities():
...
def _get_heatmap_based_on_informative_features(selector: BaseWeightSelectorWrapper, dataset: Dataset):
    weights_per_class = selector.get_weights_per_class() if SelectionSpecificity.PER_LABEL in selector.get_selection_specificities() else []
    if SelectionSpecificity.GENERAL in selector.get_selection_specificities():
...
def _create_informative_features_chart_output_by_selector(informative_scores_by_selector: dict[str, List[InformativeFeaturesScore]]):
    ...
    '''
    time_counter = ExecutionTimeCounter().print_start("Persisting informative features chart by selector...")
